...
def testBitch():
    ...
    check50.run("python3 censor.py").stdin("Hey, stop your bitching", prompt=True).stdout("Censor sentence: Hey, stop your b***hing").exit()

@check50.check(exists)
def testDamn():
    """input of \"Damn you\" yields output of \"Censor sentence: D**n you\""""
    check50.run("python3 censor.py").stdin("Damn you", prompt=True).stdout("Censor sentence: D**n you").exit()

@check50.check(exists)
def testAsshole():
    """input of \"What an asshole\" yields output of \"Censor sentence: What an a*****e\""""
    check50.run("python3 censor.py").stdin("What an asshole", prompt=True).stdout("Censor sentence: What an a*****e").exit()

@check50.check(exists)
def testShit():
    """input of \"Holy shit\" yields output of \"Censor sentence: Holy s**t\""""
    check50.run("python3 censor.py").stdin("Holy shit", prompt=True).stdout("Censor sentence: Holy s**t").exit()
...
def testMultiple():
    ...
    check50.run("python3 censor.py").stdin("Damn, you're shit out of luck!", prompt=True).stdout("Censor sentence: Damn, you're shit out of luck!").exit()
