...
def test_minus10():
    """usage unit = -10"""
    input = "10"
    output = "Enter water usage unit: "
    check50.run("python3 water.py").stdin(input, prompt=True).stdout(regex(output), output, regex=True).kill()
...
def test_50():
    ...
    check50.run("python3 coke.py").stdin(input, prompt=True).stdout(regex(output), output, regex=True).exit()
